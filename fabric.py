...
class Plugin(CliPlugin):
    def load(self, cli, **_kwargs):
        ...
        fabric.add_command(Syntax('help', help='requires uplinks, route-reflector, statistics or summary keywords'),update_location=False, callback=self._show_help)
        fabric.add_command(Syntax('summary', help='shows uplinks, route-reflector and statistics all together'), update_location=False, callback=self._show_summary, schema=self._get_schema())
        fabric.add_command(Syntax('platform', help='shows platform information in text'), update_location=False, callback=self._show_platform, schema=self._get_schema())
        fabric.add_command(Syntax('uplink', help='shows uplinks in a table'), update_location=False, callback=self._show_uplinks, schema=self._get_schema())
        fabric.add_command(Syntax('route-reflector', help='shows route-reflectors in a table'), update_location=False, callback=self._show_rr, schema=self._get_schema())
        fabric.add_command(Syntax('statistics', help='shows statistics in a table'), update_location=False, callback=self._show_stats, schema=self._get_schema())
    ...
    def _time_handler(self, state, dt1):
        ...
        now = datetime.fromisoformat(dt0.split(' ')[0]) # SR Linux returns date and time as ISO format +" (<human-readable elapsed time since>)"
        try:
            then = datetime.fromisoformat(dt1.split(' ')[0])
        except:
            then = now
    ...
    def _populate_data_platform(self, result, state):
        ...
        self._fetch_state_platform(state)
    ...
    def _populate_interface_list(self, state, description):
        ...
        interfaces = []
        for intf in int_list_data.interface.items():
            if description in intf.subinterface.get().description:
                interfaces.append(f'{intf.name}.{intf.subinterface.get().index}')

        if not interfaces:
            print(f"No subinterface has <<{description}>> in its description! \n\n PLEASE SET THE CORRECT PARAMETERS WITH 'set-show-fabric.sh' OR DIRECTLY IN THE '/etc/opt/srlinux/cli/plugins/fabric.py' FROM THE BASH!\n")
        return interfaces

    def _populate_data_uplink(self, result, state):
        uplink_peer_list = self._populate_peer_list(state, uplink_peer_group)
        result.synchronizer.flush_fields(result)
        interfaces = self._populate_interface_list(state, description)
        data = result.uplink_header.create()
        for idx, interface in enumerate(interfaces):
            self._fetch_state_uplink(state, uplink_peer_list[idx], interface.split('.'))
            data_child = data.uplink_child.create(interfaces[idx].split('.')[0])
    ...
    def _populate_data_rr(self, result, state):
        ...
        for rr in rr_peer_list:
            ...
            data_child.rx_active_tx = f"{self.rr_data.network_instance.get().protocols.get().bgp.get().neighbor.get().afi_safi.get('evpn').received_routes}/"\
                                        f"{self.rr_data.network_instance.get().protocols.get().bgp.get().neighbor.get().afi_safi.get('evpn').active_routes}/"\
                                        f"{self.rr_data.network_instance.get().protocols.get().bgp.get().neighbor.get().afi_safi.get('evpn').sent_routes}" or 'Unknown'
    ...
    def _populate_data_stats(self, result, state):
        ...
        for interface in interfaces:
            self._fetch_state_stats(state, interface.split('.')[0])
            ...
            try:
                data_child.transceiver_volt =  self.gen_data.interface.get().transceiver.get().voltage.get().latest_value or 'N/A'
            except:
                data_child.transceiver_volt = 'N/A'
            data_child.synchronizer.flush_fields(data_child)
